...
5. **AI Brain (Hybrid Engine):** `exifr` injects embedded telescope telemetry (Make, Target, Coordinates) natively into the admin-toggled AI pipeline (**Ollama LLaVA** for local or **AWS Bedrock Claude 3** for cloud) to classify images exactly into: 'Solar System', 'Moon', 'Sun', 'Galaxies', 'Nebula', 'Superclusters', 'Constellations', or 'Comets'.
